...
class RecDetailViewModel(application:Application):AndroidViewModel(application), CoroutineScope {
    private var job = Job()
...
//    }
    ...
    fun selectRecipeId(id:Int){
        launch {
            val db = buildRDB(getApplication())
            recDetLD.value = db.recipeDao().selectRecipe(id)
        }
    }

    fun editRecipe(recName:String, descript:String, detail:String, imgUrl:String, id:Int) {
        launch {
            val db = buildRDB(getApplication())
            db.recipeDao().updateEditRec(recName, descript, detail, imgUrl, id)
        }
    }

    fun createRecipe(recipes:Recipes) {
        launch {
            val db = buildRDB(getApplication())
